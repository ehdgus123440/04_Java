...
public class FileService {
	 ...
	 * boolean isFile() : 현재 File 객체가 관리하는게 파일이면 
	 * true boolean isDirectory() : 현재 File 객체가 관리하는게 디렉토리 
	 * true boolean exists() : 파일/디렉토리가 존재하면 true, 아님 false 
	 * long length() : 파일 크기 반환 
	 * long lastModified() : 파일 마지막 수정일 (1970.01.01 09:00 부터 현재까지 지난 시간을
	 * ms 단위로 반환) 
	 ...
	 * 
	 * -> Java는 모든 운영체제가 호환 되기 때문에 경로 표기 방법도 모두 호환됨
	 * 
	 ...
	 */
	public void test2() {
		// \는 문자열(String)에서 Escape문자(\n, \t ...)를 위한 키워드
		// "\" 문자열을 나타내고 싶으면 "\\"두번 작성해야함
		File file = new File("\\io_test\\20240731\\파일생성txt");
		System.out.println("\\io_test\\20240731\\파일생성txt");
